"""
Detailed step-by-step calculation table for all 83 Java files.
...
for fp in java_files:
    ...
    subj  = next((p for p in path_parts if p.startswith('Subject')), '')
    task  = next((p for p in path_parts if p.startswith('Task')),    '')

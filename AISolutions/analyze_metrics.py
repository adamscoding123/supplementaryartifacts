"""
AI Solutions (Gemini) – Code Metrics Analysis Script
=====================================================
Calculates CCN, Halstead Volume, SLOC, and Maintainability Index
for all Java files across 10 Gemini runs.
...
def analyse_file(filepath: str) -> dict:
    ...
    # Extract run from path
    parts = filepath.replace('\\', '/').split('/')
    run  = next((p for p in parts if p.startswith('GeminiRun')), 'Unknown')
